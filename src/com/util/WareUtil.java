...
public class WareUtil {
    //文件夹名称
    private static final String DIRNAME = "ware";

    //上传图片
    public static Ware upload(HttpServletRequest request) throws Exception {
        Ware ware = new Ware();
        //检查是否为上传项
            ...
            if(fileItems != null && fileItems.size() > 0) {
                ...
                for(FileItem item:fileItems) {
                    ...
                    if(!item.isFormField()) {
                        ...
                        //设置轮播图路径
                        ware.setUrl(Util.UPLOAD_PATH + "/" + DIRNAME + "/" + filename);
                        //检验上传是否为图片
